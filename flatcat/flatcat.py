#!/usr/bin/env python
"""
Morfessor 2.0 FlatCat version 2.0.
Variant with control on output lexicon size.

This version is prepared and released by Duygu Ataman for optimal usage in Neural Machine Translation.
For questions contact ataman@fbk.eu.
"""
...
class AbstractSegmenter(object):
    ...
    def _viterbi_tag_helper(self, segments,
        ...
        #print ERROR if non-Unicode character is observed.
        if (segments.__len__() == 0):
            print "Warning: Morpheme cannot be processed. Inspect your baseline model lexicon for non-Unicode morphemes. Possible characters that cause this error are: / or \."
...
class FlatcatModel(AbstractSegmenter):
    ...
    def train_batch(self,
        ...
        # Determine the regularization weight
        ...
        vocab=get_vocabulary(file(filename))
        dict_size = vocab.__len__()
        print "Vocabulary Size of Test Set: ", dict_size

        a = float(dict_size) / float(lexicon_size)


        #if a < 10:
        #    self._lexicon_coding.weight = a # set prior weight
        #    print("Regularization weight set to, ") + str(a)
        #else:
        #    self._lexicon_coding.weight = 10
        #    print("Regularization weight set to 10. ")
        ...
        for epoch in range(self._max_epochs + ml_epochs):
            self._train_epoch(lexicon_size)

            dict_size = self._interned_morphs.__len__()
            print "Lexicon size: " + str(dict_size)
            ...
            if (self._interned_morphs.__len__() <= lexicon_size + lexicon_size*0.1):
                converged = True
                if converged:
                    conv_str = 'converged'
                    print "Model has reached the desired complexity."
                    print "Lexicon size: " + str(dict_size)
            else:
    ...
    def get_cost(self):
        ...
        cost_corpus = self._corpus_coding.get_cost()
        cost_lex = self._lexicon_coding.get_cost()

        cost = cost_corpus + cost_lex
        assert cost >= 0
        if self._supervised:
    ...
    def _convergence_of_cost(self, train_func, update_func,
                            min_cost_gain=0.01,
                            max_iterations=5, lexicon_size=40000):
    ...
    def _train_epoch(self,lexicon_size):
        ...
        for experiment in transformation_generator:
            ...
            if best.transform is None:
                ...
                if self._supervised:
                    for morph in changed_morphs:
                        self._annot_coding.modify_contribution(morph, 1)
                #if self._interned_morphs.__len__() <= LEXICONSIZE:
                #    converged=1
                #    break
...
class FlatcatLexiconEncoding(baseline.LexiconEncoding):
    ...
    def __init__(self, morph_usage, weight=1.0):
    ...
    def get_cost(self): # LEXICON COST
        assert self.boundaries >= 0
        if self.boundaries == 0:
            return 0.0
        elif self.boundaries == self.tokens:
            return 1.0
...
class FlatcatEncoding(baseline.CorpusEncoding):
    ...
    def get_cost(self): # CORPUS COST
        ...
        cost = ((self.tokens * math.log(self.tokens)
                  - self.logtokensum
                  - self.logcondprobsum
